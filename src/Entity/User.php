...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=BlogPost::class, mappedBy="userr", orphanRemoval=true)
     */
    private $blogPosts;

    /**
     * @ORM\OneToMany(targetEntity=BlogPost::class, mappedBy="user1", orphanRemoval=true)
     */
    private $blogpost1;
    ...
    public function __construct()
    {
        ...
        $this->blogpost1 = new ArrayCollection();
    }
    ...
    public function removeBlogPost(BlogPost $blogPost): self
    {
        if ($this->blogPosts->removeElement($blogPost)) {
            // set the owning side to null (unless already changed)
            if ($blogPost->getUserr() === $this) {
                $blogPost->setUserr(null);
            }
        }
     ...
     * @return Collection|BlogPost[]
     */
    public function getBlogpost1(): Collection
    {
        return $this->blogpost1;
    }

    public function addBlogpost1(BlogPost $blogpost1): self
    {
        if (!$this->blogpost1->contains($blogpost1)) {
            $this->blogpost1[] = $blogpost1;
            $blogpost1->setUser1($this);
        }

        return $this;
    }

    public function removeBlogpost1(BlogPost $blogpost1): self
    {
        if ($this->blogpost1->removeElement($blogpost1)) {
            // set the owning side to null (unless already changed)
            if ($blogpost1->getUser1() === $this) {
                $blogpost1->setUser1(null);
            }
        }
